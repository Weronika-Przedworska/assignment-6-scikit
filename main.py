...
def explore_data(df):
    ...
    corr=df.corr()
    target_col=corr['Concrete Compressive Strength']
    target_col=target_col.drop('Concrete Compressive Strength')
    most_correlated_feature = target_col.abs().idxmax() # TODO: Find the feature name with highest correlation to target
...
def preprocess_data(df, test_size=0.2, random_state=42):
    ...
    dropped=df.drop('Concrete Compressive Strength', axis=1)
    X = dropped  # All columns except target
    y = df['Concrete Compressive Strength']  # Target column only
